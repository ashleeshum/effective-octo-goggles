...
class BentoBowl:
    """Create the ingredients for the users options."""

    def __init__(self):
        """Create ingredients lists."""
        self.mainslist = ["rice", "brown rice", "noodles"]
        self.vegeslist = ["stirfry", "salad", "beetroot", "edamame beans"]
        self.meatslist = ["chicken karage", "chicken karage (spicy)",
                          "chicken katsu"]
        self.meatslist2 = ["tempura", "salmon", "teriyaki chicken"]
        self.sauceslist = ["teriyaki chicken sauce", "bbq sauce",
                           "kewpie mayo"]

    def return_lists(self):
        """Return the lists so that they can be passed through other class."""
        return [self.mainslist, self.vegeslist, self.meatslist,
                self.meatslist2, self.sauceslist]


class Submit:
    """Do the commands when user presses finish button."""

    def __init__(self, layout):
        """Create root variable so that it can be retrieved from layout class.

        Can now be accessed within this submit class.
        """
        self.root = layout.root

    # making order show up on screen
    def show_order(self, name, phoneno, popup, sauce_retr, meats_retr,
                   veges_retr, main_retr):
        ...
        if user_phone.isnumeric() and user_name != "":
            ...
            bye_msg = "We will text you when your order is ready for pick up!"
            good_day = "\nHave a good day:)"
            order_details = "Mains choice: " + main_retr + "\n"
            order_details = order_details + "Vegetables choice: " + all_veges
            order_details = order_details + "\nMeats choice: " + all_meats
            order_details = order_details + "\nSauce choice: " + sauce_retr
            order_details = order_details + "\nOrder for " + user_name + "\n\n"
    ...
    def not_entered_error(self):
        """Function for popup error message for not entered sections."""
    ...
    # only need to pass throguh self as variables are in the same class
    # thus don't need to pass every single thing
    def popup(self, frm_sauces, sauce_retr, meats_retr, veges_retr, main_retr):
        frm_sauces.grid_remove()
        """Function for finalising order."""
        ...
        if main_retr == "0":
            ...
            order_btn = tk.Button(
                ...
                text="order",
                font=("Lucinds, 13"),
                command=lambda: self.show_order(enter_name, enter_phoneno,
                                                popup, sauce_retr,
                                                meats_retr, veges_retr,
                                                main_retr)
                )
...
class Commands:
    """Do the actions for steps 1, 2, 3 and 4."""

    def __init__(self, ingredients, layout):
        """Create variables that can be used from different classes."""
        ...
        # assigning the parameters to a class variable so it
        # can be accessed in other methods within this class, not just innit
    ...
    def step_4(self):
        """Functions for step 4 button section."""
        ...
        finish_button = tk.Button(
            ...
            height=1,
            font=("Lucinds, 10"),
            command=lambda:self.submit.popup(frame_sauces_section,
                                               self.saucesvar.get(),
                                               self.meatsvar,
                                               self.vegesvar,
                                               self.mainsvar.get())
        )
        finish_button.grid(row=9, column=0, columnspan=4,
                           sticky="se", pady=10, padx=10)

    def step_3(self):
        """Functions for step 3 button section.
        ...
        self.frame_meats_section.grid(row=7, column=0,
        ...
        next_button_step3.grid(row=1, column=3, columnspan=4,
                               sticky="se", pady=10, padx=10)

    def step_2(self):
        """Functions for step 2 button section.
        ...
        self.frame_vege_section.grid(row=5, column=0,
                                     columnspan=4, sticky="we")
        ...
        next_button_step2.grid(row=1, column=3, columnspan=4,
                               sticky="e", pady=10, padx=10)

    def step_1(self):
        """Drop down section 1 will occur."""
        self.frame_mains_section = tk.Frame(self.frame_header, bg="#FDFDFB")
        self.frame_mains_section.grid(row=3, column=0,
                                      columnspan=4, sticky="we")
        ...
        next_button_step1.grid(row=1, column=3, columnspan=4,
                               sticky="e", pady=10, padx=10)


class Layout:
    """Layout that is displayed when user first opens the program."""

    def __init__(self):
        """Make the basis of the program."""
        root = tk.Tk()
...
# calling layout class because that's where it starts.
Layout()
